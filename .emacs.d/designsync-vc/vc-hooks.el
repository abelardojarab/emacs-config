;;; vc-hooks.el --- DesignSync version of vc-hooks.el
...
;; $Id: vc-hooks.el.rca 1.20 Thu Dec 11 10:46:48 2003 ken Experimental $
...
(defcustom vc-sync-work-style 'unknown
  "*DesignSync work style - locking or merging.
'locking means you have to lock files before making changes.
'merging means you merge in other's changes with yours."
  :type '(choice (const merging) (const locking))
  :group 'vc)
...
(defun vc-parse-sync-status (dir)
  ;; Parse output of "dss ls -report US" command in the current
  ;; buffer and set file properties accordingly.  DIR specifies
  ;; the directory, since "ls" doesn't print it (and "ls -fullpath"
  ;; is harder to parse).
  ;; Assumes point is set at beginning of first line following
  ;; column headers.
  ;; NB: This seems quite hard to do right based on the 'ls' output
  ;; though it would be easier with rel40 and -format list.
  (let (file status locker is-locked-here)
    ;; 1st column is Locked By.
    (if (re-search-forward
         (concat "^\\([^ *]+\\)"        ; locker--anything but space, *
                 "\\([*]?\\)")          ; trailing * means is-locked-here
         nil t)
        (progn
          (setq locker (match-string 1))
          (setq is-locked-here (string= "*" (match-string 2)))))
    ;; 2nd column is Status and 3rd is Name.
    (if (re-search-forward
         (concat " +"                   ; spaces
                 "\\([A-Za-z ,-]+\\)"   ; Status
                 "  "                   ; 2 spaces separate columns
                 "\\(\\S-*\\)\\s-*\n")  ; Name
         nil t)
        (progn
          (setq status (match-string 1))
          (setq file (concat dir (match-string 2))))
      (error "Can't parse dss ls output in buffer *vc-info*"))
    ;; Set property: vc-master-locks
    (vc-file-setprop file 'vc-master-locks
                     (if locker
                         (list (list locker (vc-file-getprop
                                             file 'vc-workfile-version)))
                       'none))
    ;(message "DBG:file=%s status=%s locker=%s locks=%s" file status locker (vc-file-getprop file 'vc-master-locks))(sit-for 5)
    ;; Set property: vc-locking-user
    (vc-file-setprop file 'vc-locking-user (or locker 'none))
    ;; Set property: vc-cvs-status
    (vc-file-setprop file 'vc-cvs-status
     (cond
      ((string-match "Up-to-date"           status) 'up-to-date)
      ((string-match "Locally Modified"     status) 'locally-modified)
      ((string-match "Added by merge, Needs Checkin"
                     status)                        'locally-modified)
      ((string-match "Needs Merge"          status) 'needs-merge)
      ((string-match "Needs Update"         status) 'needs-checkout)
      ((string-match "Unresolved Conflicts" status) 'unresolved-conflict)
      (t 'unknown)))
    ))
...
(defun vc-fetch-master-properties (file)
  ...
  (save-excursion
	...
	(set-buffer (get-buffer "*vc-info*"))
     ...
     ((eq (vc-backend file) 'DesignSync)
      (save-excursion
        (vc-sync-ensure-syncd)
        ;; Call "dss ls" and parse the output in buffer *vc-info*.
        (vc-simple-command 0 "dss" file "ls" "-report" "US")
	(set-buffer (get-buffer "*vc-info*"))
        (goto-char (point-min))
        ;; Move to beginning of line after column headers line.
        ;; Last column is "Name" so headers line ends with " ----<space>\n".
        (if (re-search-forward " ----\\s-*\n" nil t)
            (vc-parse-sync-status (file-name-directory file))
          (pop-to-buffer (get-buffer "*vc-info*"))
          (error "Unable to parse output of dss ls"))))
     )
   ...
   (cond 
    ...
    ((eq (vc-backend file) 'DesignSync)
     (vc-file-setprop file 'vc-checkout-model
      (cond
       ((eq vc-sync-work-style 'locking)
        'manual)
       ((eq vc-sync-work-style 'merging)
        'implicit)
       ;; sync todo: figure out based on FetchReadOnly registry setting?
       ;; Nah, that isn't sufficient to tell.
       (t (error "Sync work style unset -- must set variable 'vc-sync-work-style")))))
    ((eq (vc-backend file) 'CVS)
     (vc-file-setprop file 'vc-checkout-model
      (cond
...
(defun vc-locking-user (file)
  ...
  (let ((locking-user (vc-file-getprop file 'vc-locking-user)))
	...
	(or (and (eq (vc-checkout-model file) 'manual)
       ...
       ((eq (vc-backend file) 'DesignSync)
        ;; Sync todo: Doing (vc-fetch-master-properties) here causes a
        ;; "dss ls", which is not wonderful, because this code is being
        ;; run as each buffer is loaded.  However, it is correct and the
        ;; commented out is not.
        ;; So-o-o-o: should detect locker by <L> in Contents file.
	...
	((eq (vc-backend file) 'CVS)
	 (if (vc-consult-rcs-headers file)   ;; CVS
	   ...
	   (vc-file-getprop file 'vc-workfile-version)))
        ((eq (vc-backend file) 'DesignSync)
         (catch 'found
           (vc-find-sync-master (file-name-directory file)
                                (file-name-nondirectory file)))
         (vc-file-getprop file 'vc-workfile-version))
        ))
...
;; vc-seconds-to-time was taken from time-date.el because this package
;; needs to run with emacs 20.  Credit for time-date.el goes to authors:
;;   Lars Magne Ingebrigtsen <larsi@gnus.org>
;;   Masanobu Umeda <umerin@mse.kyutech.ac.jp>
(defun vc-seconds-to-time (seconds)
  "Convert SECONDS (a floating point number) to a time value."
  (list (floor seconds 65536)
        (floor (mod seconds 65536))
        (floor (* (- seconds (ffloor seconds)) 1000000))))

(defun vc-find-sync-master (dirname basename)
  ;; Check if DIRNAME/BASENAME is handled by Sync.
  ;; If it is, set some properties and do a
  ;; (throw 'found (cons MASTER-FILE 'DesignSync)).
  ;; Note: This function throws the name of .SYNC/Contents file.
  ;; The function returns nil if DIRNAME/BASENAME is not handled by Sync.
  (if (and
	   (file-directory-p (concat dirname ".SYNC/"))
	   (file-readable-p (concat dirname ".SYNC/Contents")))
      (let ((file (concat dirname basename))
            buffer)
        (unwind-protect
	    (save-excursion
	      (setq buffer (set-buffer (get-buffer-create "*vc-info*")))
	      (vc-insert-file (concat dirname ".SYNC/Contents"))
	      (goto-char (point-min))
	      ;; make sure that the file name is searched 
	      ;; case-sensitively - case-fold-search is a buffer-local
	      ;; variable, so setting it here won't affect any other buffers
		 ...
		 nil t)
		;; We found it.
                ;; If <State> is L, then we have the lock.  Else, we won't
                ;; know who does until we run "dss ls".
                (if (string= "L" (match-string 1))
                    (vc-file-setprop file 'vc-locking-user (vc-file-owner file)))
                ;; Store away version number.
		(vc-file-setprop file
				 'vc-workfile-version
				 (match-string 2))
		;; If the file hasn't been modified since checkout,
		;; store the checkout-time.
		(let ((mtime (nth 5 (file-attributes file)))
                      (co-time-str (match-string 3))
                      co-time)
                  ;; Convert time_t string to emacs time value.
                  ;; Conversion must happen through float because emacs20
                  ;; on Windows can't scan time_t integers.
                  (setq co-time (vc-seconds-to-time
                                 (string-to-number (concat co-time-str ".0"))))
                  ;(message "DBG:mtime %s co-time %s" mtime co-time)(sit-for 5)
                  ;; Compare only seconds portions.
		  (if (and (= (car mtime) (car co-time))
                           (= (cadr mtime) (cadr co-time)))
		      (vc-file-setprop file 'vc-checkout-time mtime)
		    (vc-file-setprop file 'vc-checkout-time 0)))
		(throw 'found (cons (concat dirname ".SYNC/Contents") 'DesignSync)))
	       (t nil)))
	  (kill-buffer buffer)))))
...
to do that, use this command a second time with no argument."
  ...
  (if (or (and (boundp 'vc-dired-mode) vc-dired-mode)
          ...
          (and (vc-backend (buffer-file-name))
               ;; DesignSync merging style does not use vc-next-action.
               ;; Instead, let it fall through to (toggle-read-only).
               (not (and (eq (vc-backend (buffer-file-name)) 'DesignSync)
                         (eq vc-sync-work-style 'merging)))))
  ...
  (let ((file (buffer-file-name)))
    (and (vc-backend file)
	 ...
	 (or (and (eq (vc-backend file) 'CVS) 
	     ...
	     t)
	 (or (and (eq (vc-backend file) 'DesignSync)
		  (vc-file-setprop file 'vc-cvs-status nil))
	     t)
	 (vc-mode-line file))))
...
control system name."
  ...
  (let ((vc-type (vc-backend file)))
    (setq vc-mode
	  (and vc-type
               ;; kenstir: may need to merge xemacs stuff here
